...
function downloadOrderPDF(id) {
  ...
  const fmt      = v => v || '—';
  const fmtMoney = v => v != null && v !== '' ? '$' + parseFloat(v).toFixed(2) : '$0.00';
  const fmtDate  = v => v ? new Date(v).toLocaleDateString('en-US', { month:'long', day:'numeric', year:'numeric' }) : '—';
  const SIZE_ORDER = ['XS','S','M','L','XL','2XL','3XL','4XL','5XL'];

  // ── Build unified render groups ──────────────────────────────────────────
  // Handles both manual group orders and simple customer orders
  let renderGroups;
  if (o.decorationGroups && o.decorationGroups.length) {
    renderGroups = o.decorationGroups.map(g => ({
      items: g.items || [],
      decos: g.decos && g.decos.length ? g.decos
        : (g.decorationTypes || []).map((type, i) => ({
            type,
            typeLabel: type,
            location: Object.values(g.locations || {})[i] || '',
          })),
    }));
  } else {
    const decos = o.decorations && o.decorations.length
      ...
      : o.decorationType
        ? [{ type: o.decorationType, typeLabel: o.decorationType,
             locations: (o.decorationLocation || '').split(',').map(s => s.trim()).filter(Boolean) }]
        : [];
    renderGroups = [{
      items: [{
        productId:    '',
        ...
        color:        o.color   || '—',
        colorHex:     o.colorHex || '#888',
        quantities:   o.quantities || {},
        totalQty:     o.totalQty || 0,
        pricePerPiece: o.pricePerPiece,
        totalPrice:   o.totalPrice,
        mockup:       (o.mockups || [])[0]?.imageData || null,
      }],
      decos,
    }];
  }

  // ── Collect all sizes used across every item ──────────────────────────────
  const usedSizes = SIZE_ORDER.filter(sz =>
    renderGroups.some(g => g.items.some(it => (it.quantities || {})[sz] > 0))
  );
  const showSizes = usedSizes.length > 0;

  // ── Totals ────────────────────────────────────────────────────────────────
  const totalQty  = renderGroups.reduce((s, g) =>
    s + g.items.reduce((ss, it) => ss + (it.totalQty || 0), 0), 0);
  const itemTotal = parseFloat(o.totalPrice) || 0;

  // ── Artwork map (from artworkName string "Loc: file | Loc: file") ─────────
  const artworkMap = {};
  ...
  // ── Per-group HTML ────────────────────────────────────────────────────────
  let imprintCount = 0;
  const groupsHtml = renderGroups.map((group) => {
    const sizeHeads = showSizes ? usedSizes.map(sz => `<th class="sz">${sz}</th>`).join('') : '';
    const colSpan   = 4 + (showSizes ? usedSizes.length : 0) + 4;

    const rowsHtml = group.items.map(item => {
      const sizeCells = showSizes
        ? usedSizes.map(sz => { const q = (item.quantities||{})[sz]; return `<td class="sz">${q > 0 ? q : ''}</td>`; }).join('')
        : '';
      const thumb = item.mockup
        ? `<img src="${item.mockup}" class="item-img" alt="">`
        : '';
      return `
        <tr class="item-row">
          <td class="td-cat"></td>
          <td class="td-num">${item.productId || ''}</td>
          <td class="td-color">${item.color || '—'}</td>
          <td class="td-desc">${item.productName || '—'}</td>
          ${sizeCells}
          <td class="td-qty">${item.totalQty || 0}</td>
          <td class="td-items">${item.totalQty || 0}</td>
          <td class="td-price">${item.pricePerPiece ? '$' + parseFloat(item.pricePerPiece).toFixed(2) : '—'}</td>
          <td class="td-total">${item.totalPrice ? '$' + parseFloat(item.totalPrice).toFixed(2) : '—'}</td>
        </tr>
        ${thumb ? `<tr><td colspan="${colSpan}" style="padding:4px 8px 10px 8px">${thumb}</td></tr>` : ''}`;
    }).join('');

    const imprintBoxes = (group.decos || []).map(d => {
      imprintCount++;
      const typeLabel = d.typeLabel || d.type || '—';
      const locs = Array.isArray(d.locations) ? d.locations.join(', ') : (d.location || '—');
      const artLines = (Array.isArray(d.locations) ? d.locations : [d.location]).filter(Boolean).map(loc => {
        const art = artworkForLoc(loc);
        return art ? `<div class="imp-art">Artwork (${loc}): ${art}</div>` : '';
      }).join('');
      return `<div class="imprint-box">
        <div class="imp-id">IMPRINT #${o.id}-${imprintCount}</div>
        <div class="imp-name">${typeLabel}${locs !== '—' ? ' · ' + locs : ''}</div>
    ...
    return `<div class="group-block">
      ...
      ${imprintBoxes ? `<div class="imprint-row">${imprintBoxes}</div>` : ''}
    </div>`;
  }).join('');

  // ── Mockups (all order-level mockups, not tied to a group) ────────────────
  const orderMockups = (o.mockups || []).filter(m => m.imageData);
  const mockupsHtml = orderMockups.length
    ? `<div class="mockup-section">
        <div class="section-sep-title">Mockups</div>
        <div class="mockup-row">${orderMockups.map(m => `
          <div class="mockup-block">
            <img src="${m.imageData}" alt="${m.label || ''}">
            <div>${m.label || ''}</div>
          </div>`).join('')}
        </div>
      </div>`
  ...
  // ── Notes ─────────────────────────────────────────────────────────────────
  const notesHtml = (o.notes || o.customerNote) ? `
    <div class="notes-section">
      ${o.notes       ? `<div class="section-sep-title">Customer Notes</div><div class="notes-box">${o.notes}</div>`       : ''}
      ${o.customerNote ? `<div class="section-sep-title">Note to Customer</div><div class="notes-box">${o.customerNote}</div>` : ''}
    </div>` : '';
...
body { font-family: -apple-system, Helvetica Neue, Arial, sans-serif; font-size: 12px; color: #222; background: #f0f0f0; }
a { color: #00a87e; }

/* ── Action bar (hidden on print) ── */
.action-bar { display: flex; justify-content: flex-end; gap: 8px; padding: 12px 20px; background: #fff; border-bottom: 1px solid #ddd; position: sticky; top: 0; z-index: 10; }
.action-bar button { padding: 7px 16px; border-radius: 6px; border: 1px solid #ddd; background: #fff; font-size: 12px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 6px; }
.action-bar button:hover { background: #f5f5f5; }
.action-bar .btn-dl { background: #222; color: #fff; border-color: #222; }
.action-bar .btn-dl:hover { background: #000; }

/* ── Page wrapper ── */
.page { background: #fff; max-width: 960px; margin: 24px auto; padding: 40px 44px; border-radius: 4px; }

/* ── Invoice header ── */
.inv-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 28px; }
.inv-num { font-size: 26px; font-weight: 900; letter-spacing: -.3px; }
.inv-sub { font-size: 13px; color: #666; margin-top: 4px; }
.inv-right { text-align: right; }
.paid-badge { display: inline-block; border: 2px solid #e55; color: #e55; font-weight: 800; font-size: 13px; letter-spacing: .06em; padding: 2px 12px; border-radius: 4px; }
.thank-you { font-size: 11px; color: #888; margin-top: 6px; font-style: italic; }

/* ── Company + dates section ── */
.company-block { display: grid; grid-template-columns: 80px 1fr 260px; gap: 20px; align-items: start; border: 1px solid #e0e0e0; border-radius: 6px; padding: 20px; margin-bottom: 20px; }
.logo-box { width: 72px; height: 72px; background: #111; border-radius: 8px; display: flex; align-items: center; justify-content: center; color: #00c896; font-size: 22px; font-weight: 900; letter-spacing: -.5px; }
.company-info { font-size: 12px; line-height: 1.8; }
.company-info strong { font-size: 13px; display: block; margin-bottom: 2px; }
.dates-table { width: 100%; border-collapse: collapse; font-size: 12px; }
.dates-table td { padding: 4px 8px; border-bottom: 1px solid #f0f0f0; }
.dates-table td:first-child { color: #888; font-weight: 500; text-align: right; }
.dates-table td:last-child { font-weight: 600; text-align: right; }
.dates-table tr:last-child td { border-bottom: none; }

/* ── Customer section ── */
.customer-block { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 24px; }
.customer-col-title { font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: .08em; color: #888; margin-bottom: 8px; }
.customer-col { font-size: 12px; line-height: 1.8; }

/* ── Line items table ── */
.items-table { width: 100%; border-collapse: collapse; margin-bottom: 0; font-size: 11.5px; }
.items-table thead tr { background: #f7f7f7; }
.items-table th { padding: 8px 7px; text-align: left; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: .05em; color: #888; border-bottom: 2px solid #e8e8e8; white-space: nowrap; }
.items-table td { padding: 7px 7px; border-bottom: 1px solid #f0f0f0; vertical-align: middle; }
.items-table .item-row:hover { background: #fafafa; }
.th-cat  { width: 60px; }
.th-num  { width: 60px; }
.th-color { width: 90px; }
.th-desc { }
.sz      { text-align: center; width: 34px; }
.td-qty,.td-items { text-align: center; width: 60px; font-weight: 600; }
.td-price,.td-total { text-align: right; width: 70px; white-space: nowrap; }
.td-total { font-weight: 700; }
.item-img { max-width: 70px; max-height: 70px; border: 1px solid #eee; border-radius: 4px; }

/* ── Imprint boxes ── */
.imprint-row { display: flex; flex-wrap: wrap; gap: 8px; padding: 12px; background: #f9f9f9; border-top: 1px solid #eee; }
.imprint-box { background: #fff; border: 1px solid #e0e0e0; border-radius: 5px; padding: 10px 14px; min-width: 170px; font-size: 11px; }
.imp-id { font-size: 9px; font-weight: 800; text-transform: uppercase; letter-spacing: .07em; color: #aaa; margin-bottom: 3px; }
.imp-name { font-weight: 700; font-size: 12px; margin-bottom: 5px; }
.imp-row { color: #555; line-height: 1.6; }
.imp-art { color: #555; line-height: 1.6; font-style: italic; }
...
.mockup-row { display: flex; flex-wrap: wrap; gap: 14px; margin-top: 10px; }
.mockup-block { text-align: center; font-size: 10px; color: #888; }
.mockup-block img { max-width: 130px; max-height: 130px; border: 1px solid #ddd; border-radius: 4px; display: block; margin-bottom: 4px; }

/* ── Notes ── */
.notes-section { margin-bottom: 20px; }
.notes-box { background: #f9f9f9; border-left: 3px solid #00a87e; padding: 9px 13px; border-radius: 3px; font-size: 12px; line-height: 1.5; margin-top: 6px; margin-bottom: 12px; }

/* ── Section separator title ── */
.section-sep-title { font-size: 10px; font-weight: 800; text-transform: uppercase; letter-spacing: .08em; color: #aaa; margin: 16px 0 6px; }
...
.totals-table .total-due-row td { font-size: 14px; font-weight: 800; border-top: 2px solid #222; border-bottom: none; padding-top: 8px; }
.totals-table .outstanding-row td { font-weight: 700; }
.totals-table .outstanding-row td:last-child { color: ${o.isPaid ? '#00a87e' : '#e55'}; }

/* ── Payments ── */
.payments-block { border-top: 1px solid #eee; padding-top: 16px; margin-bottom: 20px; }
.payments-title { font-size: 14px; font-weight: 700; margin-bottom: 12px; }
.payment-row { display: flex; justify-content: space-between; align-items: center; padding: 8px 0; border-bottom: 1px solid #f5f5f5; }
.payment-amount { font-size: 16px; font-weight: 800; color: #00a87e; }
...
@media print {
  ...
  body { background: #fff; }
  .page { margin: 0; padding: 20mm 16mm; border-radius: 0; max-width: 100%; }
...
<div class="page">
  ...
  <div class="inv-header">
    <div>
      <div class="inv-num">Invoice #${o.id}</div>
      <div class="inv-sub">${fmt(o.customerCompany || o.customerName)}</div>
    </div>
    <div class="inv-right">
      ${o.isPaid ? '<div class="paid-badge">PAID</div><div class="thank-you">Thank you for your business!</div>' : ''}
    </div>
  </div>
  ...
  <div class="company-block">
    ...
    <div class="company-info">
      <strong>Insignia Screen Printing</strong>
      61297 390th Ave<br>
      Zumbro Falls, Minnesota 55991<br>
      507-900-1812<br>
      <a href="https://www.ispclothing.com">https://www.ispclothing.com</a><br>
      blake@insigniascreenprinting.com
    </div>
  ...
  <div class="customer-block">
    <div class="customer-col">
      <div class="customer-col-title">Customer Billing</div>
    ...
    <div class="customer-col">
      <div class="customer-col-title">Customer Shipping</div>
      ${o.customerCompany ? `<div>${o.customerCompany}</div>` : ''}
      <div>${fmt(o.customerName)}</div>
      ${o.salesRepName ? `<div style="margin-top:6px;color:#888;font-size:11px">Sales Rep: <strong style="color:#222">${o.salesRepName}</strong></div>` : ''}
    </div>
  </div>
  ...
  <div class="totals-block">
    <table class="totals-table">
      ...
      <tr class="total-due-row"><td>Total Due</td><td>${fmtMoney(itemTotal)}</td></tr>
      ${o.isPaid ? `<tr><td>Paid</td><td>${fmtMoney(itemTotal)}</td></tr>` : ''}
      <tr class="outstanding-row"><td>Amount Outstanding</td><td>${fmtMoney(o.isPaid ? 0 : itemTotal)}</td></tr>
    </table>
  </div>
  ...
  <div class="payments-block">
    ...
    <div class="payment-row">
      <span class="payment-amount">${fmtMoney(itemTotal)}</span>
      <span class="payment-date">${fmtDate(o.updatedAt || o.createdAt)}</span>
    </div>
  </div>` : ''}
